...
def generate_confidence_graph(confidence, prediction_result):
    try:
        ...
        if prediction_result == "FAKE":
            colors = [fake_cmap(0.6), '#95a5a6']
            labels = ['Fake', 'Real']
            sizes = [confidence, 100 - confidence]
            explode = (0.05, 0)
        else:
            colors = [real_cmap(0.6), '#95a5a6']
            labels = ['Real', 'Fake']
            sizes = [confidence, 100 - confidence]
            explode = (0.05, 0)
        ...
        plt.title('Confidence Score', pad=20, fontsize=16, fontweight='bold', color='white')
        plt.axis('equal')
        ...
        plt.savefig(graph_path, bbox_inches='tight', dpi=300, transparent=True, facecolor='#1a1a1a')
        plt.close()
